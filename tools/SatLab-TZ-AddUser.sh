...
function set_access_groups() {
	...
	ibmcloud iam access-group-user-add ${SAT_ACCESS_GROUP} ${USERID} || errorOut "failed adding ${USERID} to ${SAT_ACCESS_GROUP}"
...
function set_access_policies() {
	...
	do
		...
		ibmcloud iam user-policy-create ${USERID} --roles Writer --service-name "containers-kubernetes" --service-instance ${cluster} --attributes namespace=razeedeploy || errorOut "failed creating containers-kbernetes writer policy for razeedeploy namespace for user ${USERID}"
		...
		ibmcloud iam user-policy-create ${USERID} --roles Administrator,Manager --service-name "containers-kubernetes" --service-instance ${cluster} --attributes namespace=${USER_NAMESPACE} || errorOut "failed creating containers-kbernetes Administrator,Manager policy for ${USER_NAMESPACE} namespace for user ${USERID}"
	...
	ibmcloud iam user-policy-create ${USERID} --roles Editor,Manager,Administrator --service-name "satellite" --resource-type  "configuration" --resource ${USER_NAMESPACE} || errorOut "failed creating satellite Editor,Manager,Administrator policy for configuration for resource ${USER_NAMESPACE} for user {$USERID}"
